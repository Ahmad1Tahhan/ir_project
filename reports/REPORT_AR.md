...
> الفريق مكوّن من **5 أعضاء** (بعد انسحاب أحد الأعضاء وُزّعت مهامه على البقية). تم تحقيق
> **4 ميزات إضافية** (أكثر من الحد المطلوب)، وكل ميزة **قابلة للاختبار بشكل مستقل**.
...
> الفريق **5 أعضاء**. مهام العضو المنسحب (الواجهة + بوابة API + الخدمات) أُعيد توزيعها على
> البقية بحيث يملك كل عضو غلاف الخدمة (Service) الموافق للجزء الذي بناه.

| العضو | المهمة |
|---|---|
| احمد الطحان | المعالجة المسبقة + خدمة Preprocessing + **بوابة API (Gateway)** |
| محمد دريد فاخوري | التمثيلات TF-IDF / BM25 + الفهرس المعكوس + **تحسين الاستعلام + خدمة Refinement** |
| طارق المزيك | الـ Embeddings + FAISS + الهجين + **خدمة Retrieval** |
| ياسر جمال الدين | التقييم + الرسومات + النوتبوك + **خدمة Evaluation** |
| معتز الحلاق | الميزات الإضافية (Clustering / Topics / RAG + خدمة RAG) + التقرير + **واجهة Streamlit** |
